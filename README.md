...
Let $Q(P, M, S)$ denote the QPROP function, which takes propeller parameters $P$, motor parameters $M$, and setpoint parameters $S$ to return the state of the plane at the current setpoint. A single process runs several of these to obtain the full takeoff dynamic characteristics of the plane. Let $\mathcal{T}(Q, C, M)$ denote the takeoff dynamic characteristics at mass $M$ given a configuration $C$. As alluded, each process $i$ will run its own $\mathcal{T}$, executing multiple $Q$ functions until it obtains the full takeoff dynamic characteristics of the plane. Knowing that $\mathcal{T}$ is monotonically increasing with respect to mass, thus implying that the output of $N$ processes receiving from a sorted list of masses will be sorted, binary search can be used to narrow down the search until a solution within tolerance $\epsilon$ is reached. The elaborated time complexity of this solution is $O(log_2(\frac{R}{\epsilon}))$, where $R$ is the difference between the maximum and minimum mass values.

The solution implemented in the script is an improvement over binary search, as it utilizes a parallel n-interval (i.e. n-ary) search, where $n = N+1$ and $N$ is the number of forked processes passed to the script. When using binary (2-ary) search, 1 midpoint is evaluated, splitting the search space into $\frac{1}{2}$ its original size, hence the $log_2$ asymptotic behavior. When using parallel n-ary search, $N = n-1$ midpoints are evaluated in parallel, with their results being used to split the search space into $\frac{1}{n}$ its original size, hence a $log_n$ asymptotic behavior. This solution is highly effective for the optimizer because a single process running $\mathcal{T}$ can take several minutes under low tolerances. Parallelizing the search and increasing the number of midpoints effectively reduces the search space by a factor of $log_2(n)$.
